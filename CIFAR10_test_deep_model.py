...
class VoronoiCell():
	def __init__(self, cell_id):
		self.id = cell_id
		# Linear Layers
		self.Linear_1 = nn.Linear(in_features = 256 * 4 * 4, out_features = 120)
...
class Net(nn.Module):
	def __init__(self, num_of_cells):
		...
		self.Conv2d_1 = nn.Conv2d(in_channels = 3, out_channels = 6, kernel_size = (3, 3), padding=1, padding_mode='zeros')
		self.Conv2d_2 = nn.Conv2d(in_channels = 6, out_channels = 16, kernel_size = (3, 3), padding=1, padding_mode='zeros')

		self.Conv2d_3 = nn.Conv2d(in_channels = 16, out_channels = 32, kernel_size = (3, 3), padding=1, padding_mode='zeros')
		self.Conv2d_4 = nn.Conv2d(in_channels = 32, out_channels = 64, kernel_size = (3, 3), padding=1, padding_mode='zeros')

		self.Conv2d_5 = nn.Conv2d(in_channels = 64, out_channels = 128, kernel_size = (3, 3), padding=1, padding_mode='zeros')
		self.Conv2d_6 = nn.Conv2d(in_channels = 128, out_channels = 256, kernel_size = (3, 3), padding=1, padding_mode='zeros')
	...
	def forward(self, inp):
		...
		inp = F.relu(self.Conv2d_2(F.relu(self.Conv2d_1(inp))))
		inp = self.MaxPool2d(inp)
		inp = F.relu(self.Conv2d_4(F.relu(self.Conv2d_3(inp))))
		inp = self.MaxPool2d(inp)
		inp = F.relu(self.Conv2d_6(F.relu(self.Conv2d_5(inp))))
		inp = self.MaxPool2d(inp)
		inp = inp.view(-1, 512 * 5 * 5)
	...
	def compute_meta_losses(self, hypotheses, labels, criterion, min_indices, epsilon, device):
		for head_idx in range(self.number_of_Voronoi_Cells):
			delta_values = torch.ones(size=(hypotheses.size()[1], ), device=device)
...
def main():
	...
	for epoch in range(1, 9):
		...
		for iteration, data in enumerate(train_loader):
			...
			epsilon = 0.2
			net.compute_meta_losses(hypotheses, labels, criterion, min_indices, epsilon, device)
			optimizer.step()
