...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        self.upfeat6 = deconv(od+dd[4], 4, kernel_size=4, stride=2, padding=1)
        ...
        self.upfeat5 = deconv(od+dd[4], 4, kernel_size=4, stride=2, padding=1)
        ...
        self.upfeat4 = deconv(od+dd[4], 4, kernel_size=4, stride=2, padding=1)
        ...
        self.upfeat3 = deconv(od+dd[4], 4, kernel_size=4, stride=2, padding=1)
    ...
    def forward(self,x):
        ...
        # ADDED for ELU -- start
        elu = torch.nn.ELU()
        sigma_part = elu(flow6[:, 2:4, :, :])
        sigma_part = torch.add(sigma_part, 2)
        flow6 = torch.cat((flow6[:, 0:2, :, :], sigma_part), 1)
        # ADDED for ELU -- end

        # ADDED for multiplication with 1/minimum_number -- start
        """epsilon = 0.0001
        sigma_part_u = flow6[:, 2, :, :]
        sigma_part_v = flow6[:, 3, :, :]
        sigma_part_u = torch.abs(sigma_part_u)
        sigma_part_v = torch.abs(sigma_part_v)
        minimum_u = torch.min(sigma_part_u) + epsilon
        minimum_v = torch.min(sigma_part_v) + epsilon
        
        sigma_part_u = torch.unsqueeze(sigma_part_u, 1)
        sigma_part_v = torch.unsqueeze(sigma_part_v, 1)
        if minimum_u < 1:
            sigma_part_u = torch.add(torch.mul(sigma_part_u, 1/minimum_u), 1)
            sigma_part_v = torch.add(torch.mul(sigma_part_v, 1/minimum_v), 1)
        
        flow6 = torch.cat((flow6[:, 0:2, :, :], sigma_part_u), 1)
        flow6 = torch.cat((flow6, sigma_part_v), 1)"""
        # ADDED for multiplication with 1/minimum_number -- end
        up_flow6 = self.deconv6(flow6)
        up_feat6 = self.upfeat6(x)

        warp5 = self.warp(c25, up_flow6[:, :2, :, :]*0.625)
        ...
        # ADDED for ELU -- start
        sigma_part = elu(flow5[:, 2:4, :, :])
        sigma_part = torch.add(sigma_part, 2)
        flow5 = torch.cat((flow5[:, 0:2, :, :], sigma_part), 1)
        # ADDED for ELU -- end
        

        # ADDED for multiplication with 1/minimum_number -- start
        """epsilon = 0.0001
        sigma_part_u = flow5[:, 2, :, :]
        sigma_part_v = flow5[:, 3, :, :]
        sigma_part_u = torch.abs(sigma_part_u)
        sigma_part_v = torch.abs(sigma_part_v)
        minimum_u = torch.min(sigma_part_u) + epsilon
        minimum_v = torch.min(sigma_part_v) + epsilon
        ...
        flow5 = torch.cat((flow5[:, 0:2, :, :], sigma_part_u), 1)
        flow5 = torch.cat((flow5, sigma_part_v), 1)"""
        # ADDED for multiplication with 1/minimum_number -- end
        ...
        warp4 = self.warp(c24, up_flow5[:, :2, :, :]*1.25)
        ...
        # ADDED for ELU -- start
        sigma_part = elu(flow4[:, 2:4, :, :])
        sigma_part = torch.add(sigma_part, 2)
        flow4 = torch.cat((flow4[:, 0:2, :, :], sigma_part), 1)
        # ADDED for ELU -- end
        
    
        # ADDED for multiplication with 1/minimum_number -- start
        """epsilon = 0.0001
        sigma_part_u = flow4[:, 2, :, :]
        sigma_part_v = flow4[:, 3, :, :]
        sigma_part_u = torch.abs(sigma_part_u)
        sigma_part_v = torch.abs(sigma_part_v)
        minimum_u = torch.min(sigma_part_u) + epsilon
        minimum_v = torch.min(sigma_part_v) + epsilon
        ...
        flow4 = torch.cat((flow4[:, 0:2, :, :], sigma_part_u), 1)
        flow4 = torch.cat((flow4, sigma_part_v), 1)"""
        # ADDED for multiplication with 1/minimum_number -- end
        ...
        warp3 = self.warp(c23, up_flow4[:, :2, :, :]*2.5)
        ...
        # ADDED for ELU -- start
        sigma_part = elu(flow3[:, 2:4, :, :])
        sigma_part = torch.add(sigma_part, 2)
        flow3 = torch.cat((flow3[:, 0:2, :, :], sigma_part), 1)
        # ADDED for ELU -- end

        # ADDED for multiplication with 1/minimum_number -- start
        """epsilon = 0.0001
        sigma_part_u = flow3[:, 2, :, :]
        sigma_part_v = flow3[:, 3, :, :]
        sigma_part_u = torch.abs(sigma_part_u)
        sigma_part_v = torch.abs(sigma_part_v)
        minimum_u = torch.min(sigma_part_u) + epsilon
        minimum_v = torch.min(sigma_part_v) + epsilon
        ...
        flow3 = torch.cat((flow3[:, 0:2, :, :], sigma_part_u), 1)
        flow3 = torch.cat((flow3, sigma_part_v), 1)"""
        # ADDED for multiplication with 1/minimum_number -- end
        ...
        warp2 = self.warp(c22, up_flow3[:, :2, :, :]*5.0)
        ...
        # ADDED for ELU -- start
        """
        sigma_part = elu(flow2[:, 2:4, :, :])
        sigma_part = torch.add(sigma_part, 2)
        flow2 = torch.cat((flow2[:, 0:2, :, :], sigma_part), 1)"""
        # ADDED for ELU -- end

        # ADDED for multiplication with 1/minimum_number -- start
        """epsilon = 0.0001
        sigma_part_u = flow2[:, 2, :, :]
        sigma_part_v = flow2[:, 3, :, :]
        sigma_part_u = torch.abs(sigma_part_u)
        sigma_part_v = torch.abs(sigma_part_v)
        minimum_u = torch.min(sigma_part_u) + epsilon
        minimum_v = torch.min(sigma_part_v) + epsilon
        ...
        flow2 = torch.cat((flow2[:, 0:2, :, :], sigma_part_u), 1)
        flow2 = torch.cat((flow2, sigma_part_v), 1)"""
        # ADDED for multiplication with 1/minimum_number -- end
